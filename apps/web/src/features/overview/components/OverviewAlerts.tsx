...
export const OverviewAlerts: React.FC<OverviewAlertsProps> = ({ alerts, onSelectZoneId }) => {
  // Sort: Severity descending (Critical -> High -> Moderate -> Low), then Timestamp descending
  ...
  return (
    <div className="flex flex-col h-[460px] lg:h-[520px] bg-white rounded-[8px] border border-slate-200 shadow-sm p-4">
      {/* Panel Header */}
      <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-3 shrink-0">
        <div className="flex items-center gap-2">
          <BellRing className="w-4 h-4 text-red-600" />
          <h2 className="text-sm font-bold text-slate-900 tracking-tight">Active Alert Triage</h2>
        </div>
        ...
        {sortedAlerts.length === 0 ? (
          ...
          sortedAlerts.map((alert) => (
            <Card
              ...
              className="p-3 bg-slate-50/50 hover:bg-slate-50 border-slate-200 transition-colors"
            >
      ...
      <div className="border-t border-slate-100 pt-3 mt-2 shrink-0">
        <Link
          to="/alerts"
        ...
        >
          <span>View all active alerts & escalation history</span>
